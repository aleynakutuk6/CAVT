...
from src.data.preprocessors import CAVTPreprocessor
from src.utils.bbox_utils import intersection_over_union, intersection_over_first


class CAVT(torch.nn.Module):
    
    def __init__(self,
                 mmdet_cfg: str, weight: str, device: str,
                 preprocessor: CAVTPreprocessor=None,
                 simple_eval: bool=False, num_repeats: int=3,
                 IoU_thold: float=0.65, OR_thold: float=0.60,
                ):
        super().__init__()
        self.model = init_detector(mmdet_cfg, weight, device=device)
        self.device = device
        self.simple_eval = simple_eval
        self.OR_thold = OR_thold
        self.num_repeats = num_repeats
        self.select_larger_thold = 0.1 
        self.IoU_thold = IoU_thold
        if preprocessor is None:
            self.preprocessor = CAVTPreprocessor()
        else:
    ...
    def forward(
        ...
        (scene_visuals, 
         ...
         stroke_areas, 
         stroke_area_inds, 
         segmentation_sizes) = self.preprocessor(
            scene_strokes, 
            divisions, 
            img_sizes)
    ...
    def postprocess(
        ...
        for b in range(b_scores.shape[0]):
            ...
            if self.simple_eval:
                ...
                while not the_same and total_iter < self.num_repeats: 
                    
                    the_same = True
                    total_iter += 1
                    # start from the smallest box to largest
                    box_areas = boxes[:, 2] - boxes[:, 0]
                    box_areas *= boxes[:, 3] - boxes[:, 1]
                    ...
                    covered_strokes = np.zeros(
                        (int(stroke_area_inds[-1, -1].data.item())))
                    # make the first assignment
                    (new_scores, 
                     new_boxes, 
                     new_ranges) = self.assign_strokes_to_closest_boxes(
                        boxes, scores, covered_strokes, 
                        stroke_areas, stroke_area_inds)

                    self.assign_non_classified_strokes_to_neighbors(
                        new_boxes, new_ranges, covered_strokes, 
                        stroke_areas, stroke_area_inds)
                    ...
                    (new_scores, 
                     new_boxes, 
                     new_ranges, 
                     covered_strokes) = self.define_objects_from_non_classified_strokes(
                        new_scores, new_boxes, new_ranges, 
                        covered_strokes, stroke_areas, stroke_area_inds)
                ...
                ranges = ranges[order, :]
                out_range = ranges[:, 0].cpu().numpy().tolist() 
                out_range = out_range + [ranges[-1, -1].data.item()]
                
                pad_start = torch.where(scene_strokes[b, :, -1] < 0)[0]
                if len(pad_start) > 0:
                    pad_start = pad_start[0] 
                else:
                    pad_start = scene_strokes.shape[-2]
    ...
    def assign_strokes_to_closest_boxes(
        ...
        for b in range(boxes.shape[0]):
            ...
            for s in range(stroke_areas.shape[0]):
                ...
                if (max_iou - iou_val <= self.select_larger_thold and 
                    covered_strokes[st:end].sum() == 0):
        ...
        return new_scores, new_boxes, new_ranges
        ...
        while changed:
            ...
            for s in range(stroke_area_inds.shape[0]):
                ...
                for b in range(new_boxes.shape[0]):
                    rngs = new_ranges[b, :]
                    # has to be adjacent to assigned strokes
                    if rngs[0] != end and rngs[1] != st: continue 
                    
                    iof = io_first[s, b]
                    max_side = max(xmax - xmin, ymax - ymin)
                    min_side = min(xmax - xmin, ymax - ymin)
                    if iof > self.OR_thold or (iof > 0 and max_side / min_side > 5):
                        # either the overlap of the stroke object has to be large
                        # or the shape of the stroke object has to be 
                        # too long or too wide (like table legs)
    ...
    def remove_small_boxes(self, new_scores, new_boxes, new_ranges, W, H):
        ...
        for el in small_boxes:
            if el == 0:
                ...
                new_boxes = torch.cat(
                    [new_boxes[:el, :], new_boxes[el+1:, :]], dim=0)
                new_scores = torch.cat(
                    [new_scores[:el], new_scores[el+1:]])
                new_ranges = torch.cat(
                    [new_ranges[:el, :], new_ranges[el+1:, :]], dim=0)


    def define_objects_from_non_classified_strokes(
        self, new_scores, new_boxes, new_ranges, 
        covered_strokes, stroke_areas, stroke_area_inds):
                ...
                for si in range(stroke_area_inds.shape[0]):
                    if stroke_area_inds[si, 0] == st and stroke_area_inds[si, 1] == s:
                        if new_boxes.shape == 0:
                            ...
                            new_boxes = torch.cat(
                                [new_boxes, stroke_areas[si:si+1, :]], dim=0)
                            new_ranges = torch.cat(
                                [new_ranges, stroke_area_inds[si:si+1, :]], dim=0)
                            new_scores = torch.cat(
                                [new_scores, torch.full((1, ), fill_value=-1)])
