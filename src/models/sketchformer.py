...
class SketchFormer:
    ...
    def __init__(self):
        ...
        tokenizer = Tokenizer(
            f"{self.root_repo}/prep_data/sketch_token/token_dict.pkl")
        tokenizer.max_seq_len = 200
    
        with open(f"{self.root_repo}/qd_list.txt", "r") as f:
            lines = f.readlines()
    ...
    def __call__(self, abs_stroke3, divisions):
        ...
        assert divisions[0] == 0
        assert start_dict[divisions[-1]] == abs_stroke3.shape[0]
        ...
        for i in range(len(divisions)-1):
            ...
            sketch_encoded = self.tokenizer.encode(sketch_temp)
            encoded_list = np.asarray([sketch_encoded])
            
            pred = self.model.encode_from_seq(encoded_list)["class"].numpy()
            pred = pred[0, :].argsort()[::-1]
            pred_names.append([self.labels_info[val] for val in pred])
