...
def draw_sketch(
    ...
    for c_idx, (class_name, fill_color) in enumerate(name_color_list):
        ...
        oh = (H - 15) - 30 * c_idx
        canvas = cv2.circle(
            canvas, (ow - 20, oh - text_size[1] // 2), 5, fill_color, -1)
        canvas = cv2.putText(
...
def visualize_from_mtx(
    ...
    for h in range(H):
        for w in range(W):
            ...
            if pred_val != 0:
                ...
                color = color_mappings[pred_name]
    ...
    for c_idx, class_name in enumerate(valid_class_set):
        ...
        vis_arr = cv2.circle(
            ...
            5, color_mappings[class_name], -1)
        vis_arr = cv2.putText(
            vis_arr, class_name, (ow, oh), 
            cv2.FONT_HERSHEY_SIMPLEX, 0.7, [0, 0, 0], 2, cv2.LINE_AA)
    
    if save_path is not None:
        cv2.imwrite(save_path, vis_arr)
