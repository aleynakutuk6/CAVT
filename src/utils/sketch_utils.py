...
def absolute_to_relative(
    sketch: np.ndarray, start_from_zero: bool=True) -> np.ndarray:
...
def strokelist_to_stroke3(
    strokelist: list, return_is_absolute: bool=False) -> np.ndarray:
    """ Makes conversion:
...
def rotate_sketch(
    sketch: np.ndarray, angle: int, is_absolute: bool=False) -> np.ndarray:
...
def apply_RDP(sketch: np.ndarray, is_absolute: bool=False, epsilon: float=2.0):
    ...
    simplified_sketch = strokelist_to_stroke3(
        new_lines, return_is_absolute=is_absolute)
...
def shift_to_origin(sketch: np.ndarray, is_absolute: bool=False):
    """
    Shifts the sketch's topleft side to origin (0, 0). Returns both
    the sketch and its W x H dimensions.
    """
    if is_absolute:
        ...
        xmin, ymin, xmax, ymax = get_relative_bounds(sketch)
        sketch[0, 0] -= xmin
        sketch[0, 1] -= ymin
    return sketch, [xmax - xmin, ymax - ymin]
